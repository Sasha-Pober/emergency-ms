    ...
    : ControllerBase
{
    ...
    [Consumes("multipart/form-data")]
    [Authorize(Roles = "Manager")]
    public async Task<IActionResult> UpdateEmergency(int id, [FromForm] UpdateEmergency request)
    {
        if (request == null)
        {
            return BadRequest("Invalid request");
        ...
        var emergency = request.MapToDTO();
        emergency.Id = id;
        await emergencyService.UpdateEmergency(emergency);


        if (request.Images != null && request.Images.Count > 0)
        {
            await imageService.UploadImages(request.Images, Request.Scheme, Request.Host.Value, emergency.Id);
        }

        if (request.ImagesToDelete != null && request.ImagesToDelete.Count > 0)
        {
            await imageService.DeleteImages(emergency.Id, request.ImagesToDelete);
        }
